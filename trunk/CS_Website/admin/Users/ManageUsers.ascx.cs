...
namespace DotNetNuke.Modules.Admin.Users
{
    ...
    public partial class ManageUsers : UserModuleBase, IActionable
    {
        ...
        protected string ReturnUrl
        {
            get
            {
                return Globals.NavigateURL( TabId, "", ( UserFilter != "" ? "filter=" + UserFilter : "" ).ToString() );
            }
        }
        ...
        private void BindData()
        {
            if( User != null )
            {
                ...
                {
                    AddModuleMessage( "SuperUser", ModuleMessage.ModuleMessageType.YellowWarning, true );
                    DisableForm();
                    return;
                }
                ...
                else
                {
                    if( ! IsUser )
                    {
                        if( Request.IsAuthenticated )
                        {
                            ...
                            {
                                Response.Redirect( Globals.NavigateURL( PortalSettings.UserTabId, "", "UserID=" + UserInfo.UserID.ToString() ), true );
                            }
                            else
                            {
            ...
            try
            {
                if( e.CreateStatus == UserCreateStatus.Success )
                {
                    ...
                    if( IsRegister )
                    {
                        ...
                        AddLocalizedModuleMessage( strMessage, ModuleMessage.ModuleMessageType.GreenSuccess, ( strMessage.Length > 0 ) );
                    }
                    else
                    {
                        if( e.Notify )
