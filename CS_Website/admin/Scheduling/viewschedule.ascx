<%@ Control Language="C#" AutoEventWireup="false" Explicit="True" Inherits="DotNetNuke.Modules.Admin.Scheduling.ViewSchedule" CodeFile="ViewSchedule.ascx.cs" %>
...
<ItemTemplate>
                <asp:IMAGE id="Image1" ImageUrl="~/images/checked.gif" Visible='<%# Convert.ToBoolean(DataBinder.Eval(Container.DataItem,"Enabled"))==true %>' AlternateText="Enabled" runat="server" resourcekey="Enabled.Header" />
...
<ItemTemplate>
                <asp:label CssClass="Normal" ID="lblNextStart" runat="server" Visible='<%# Convert.ToBoolean(DataBinder.Eval(Container.DataItem,"Enabled")) == true %>'><%# DataBinder.Eval(Container.DataItem,"NextStart")%></asp:label>
